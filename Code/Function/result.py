...
warnings.filterwarnings(
    "ignore", message="Ignoring cached namespace 'hdmf-common'")
warnings.filterwarnings(
    "ignore", message="Ignoring cached namespace 'hdmf-experimental'")
...
class PCA:
    def __init__(self, group):
        ...
        self.score = group.pca.score
        self.loading = group.pca.loading
        self.variance = group.pca.variance
    ...
    def Plot_Neural_Data(self, gap_idx, PC_idx):
        sort_idx = np.argsort(self.loading[PC_idx])[::-1]
    ...
    def Plot_Components_Correlation(self):
        ...
        corre_on, corre_off = [], []
        for i in range(10):
            corre_on.append(abs(np.corrcoef(self.group.pca.loading[i], self.group.unit_onset)[0,1]))
            corre_off.append(abs(np.corrcoef(self.group.pca.loading[i], self.group.unit_offset)[0,1]))
    ...
    def Plot_Trajectory_3d(self, PC = [0,1,2]):
        ...
        for i in range(len(self.gaps)):
            ...
            axs_[0].set_title('Pre-Gap Noise', fontsize=24)
            axs_[1].set_title('Gap', fontsize=24)
            axs_[2].set_title('Post-Gap Noise', fontsize=24)
                ...
                for k in range(5):
                    if k-1 != j:
                        axs[rol_idx+j][col_idx].plot(x[starts[k]:ends[k]], y[starts[k]:ends[k]], z[starts[k]:ends[k]], 
                                                    ls=linestyle[k], c=color[k], linewidth = 3)
                    else:
                        axs[rol_idx+j][col_idx].plot(x[starts[k]:ends[k]], y[starts[k]:ends[k]], z[starts[k]:ends[k]], 
                                                    ls=linestyle[k], c=color[k], linewidth = 3, label = labels[k-1])
    ...
    def Plot_Trajectory_3d_Event(self, PC):
        ...
        gap_idx = 0
        gap_type = 0
        gap_dur = round(gap_type/5*1000+350)
        ...
        for j in range(1,6):
            ...
            axs[0][j-1].plot(x[100:100+j*step], y[100:100+j*step], z[100:100+j*step], 
                             ls='-', linewidth = 3, c='darkgreen', label = 'Noise')
            ...
            axs[1][j-1].plot(x[gap_dur+100:gap_dur+100+j*step], y[gap_dur+100:gap_dur+100+j*step], z[gap_dur+100:gap_dur+100+j*step], 
                             ls='-', linewidth = 3, c='black', label = 'Silence')
        ...
        axs[0].set_title('Noise', fontsize=24)
        axs[1].set_title('Post-Noise Silence', fontsize=24)
        plt.tight_layout()
    ...
    def Calculate_Angle(self, PC):
        ...
        for gap_idx, gap_type in enumerate(self.gaps):
            ...
            angle_noise.append(calculate_vector_angle(pre_gap, post_gap))
            angle_gap.append(calculate_vector_angle(pre_gap, gap))
            angle_fix_gap.append(calculate_vector_angle(pre_gap, post_noise))
    ...
    def Plot_Step_Degree(self, PC):
        ...
        for i in range(len(x[100:350])):
            ...
            angle_pre_gap.append(calculate_vector_angle(start_point-end_point, point-mid_point))
        ...
        for i in range(len(x[350:gap_dur])):
            ...
            angle_gap.append(calculate_vector_angle(start_point-end_point, point-mid_point))
        ...
        for i in range(len(x[gap_dur:gap_dur+100])):
            ...
            angle_post_gap.append(calculate_vector_angle(start_point-end_point, point-mid_point))
        ...
        for i in range(len(x[gap_dur+100:])):
            ...
            angle_fix_gap.append(calculate_vector_angle(start_point-end_point, point-mid_point))

        fig, axs = plt.subplots(1, 1, figsize = (5, 5))
        axs.plot(np.arange(len(angle_pre_gap)), angle_pre_gap, label = 'Pre-Gap', color = 'green')
        axs.plot(np.arange(len(angle_gap)), angle_gap, label = 'Gap', color = 'black')
        axs.plot(np.arange(len(angle_post_gap)), angle_post_gap, label = 'Post-Gap', color = 'lightgreen')
        axs.plot(np.arange(len(angle_fix_gap)), angle_fix_gap, label = 'Silence', color = 'grey')
    ...
    def Plot_Principal_Angle(self, dim = 5):
        ...
        for gap_type in [4,7,9]:
            ...
            axs[count].set_xticklabels(['Pre-N', 'Post-N', 'Gap', 'Silence'], rotation = 0, fontsize = 14)
            axs[count].set_yticklabels(['Pre-N', 'Post-N', 'Gap', 'Silence'], fontsize = 14)
    ...
    def Plot_OnOff_Period(self):
        ...
        onset = self.group.pop_response_stand[:,gap_idx,100:200] # first 100 ms of pre-gap noise
        offset = self.group.pop_response_stand[:,gap_idx,100+gap_dur:100+gap_dur+150] # first 150 ms of post-gap silence
        noise = self.group.pop_response_stand[:,gap_idx,250:350] # last 100 ms of pre-gap noise
        ...
        onset_exclude_noise_pca = neuron.PCA(onset_exclude_noise, multiple_gaps = False)
        offset_exclude_noise_pca = neuron.PCA(offset_exclude_noise, multiple_gaps = False)
        onset_pca = neuron.PCA(onset, multiple_gaps = False)
        offset_pca = neuron.PCA(offset, multiple_gaps = False)
        gap_pca = neuron.PCA(gap, multiple_gaps = False)
    ...
    def Plot_Gap_Dependent_On_Response(self):
        ...
        for count in range(6):
            gap_dur = round(self.group.gaps[gap_idx[count]]*1000+350)

            onset = self.group.pop_response_stand[:, gap_idx[count],100:200] # first 100 ms of pre-gap noise
            offset = self.group.pop_response_stand[:,gap_idx[count],gap_dur + 100: gap_dur + 100 + 100] # first 100 ms of post-gap silence
            onset_post = self.group.pop_response_stand[:,gap_idx[count],gap_dur:gap_dur+100] # first 100 ms of post-gap noise
            onset_post_no_onset = onset_post - onset
            onset_post_no_both = onset_post - offset - onset
            
            periods = [onset, onset_post, onset_post_no_onset, onset_post_no_both, offset]
            sim = np.zeros((5,5))
            for i in range(5):
                for j in range(5):
                    period1, period2 = periods[i], periods[j]
            ...
            axs[count].set_aspect('auto')
            axs[count].set_xticklabels(['PreN', 'PostN', 'PostN-On', 'PostN-Both', 'Silence'], rotation = 0, fontsize = 16)
            axs[count].set_yticklabels(['PreN', 'PostN', 'PostN-On', 'PostN-Both', 'Silence'], fontsize = 16)
            axs[count].set_title('Gap = ' + str(round(self.group.gaps[gap_idx[count]]*1000)) +'ms', fontsize = 24)
        plt.tight_layout()
        ...
        for count in range(3):
            ...
            left_axes[count].set_xticklabels(['PreN', 'PostN', 'PostN-On', 'PostN-Both', 'Silence'], rotation = 0, fontsize = 13)
            left_axes[count].set_yticklabels(['PreN', 'PostN', 'PostN-On', 'PostN-Both', 'Silence'], fontsize = 13)
            left_axes[count].set_ylabel('Gap = ' + str(round(self.group.gaps[gap_idx[count]]*1000)) +'ms', fontsize = 24)
            
            data = self.group.pop_response_stand[:, gap_idx[count], :]
            onset_pca = neuron.PCA(onset, multiple_gaps=False)
            onset_post_pca = neuron.PCA(onset_post, multiple_gaps=False)
            ...
            sns.heatmap([self.group.gaps_label[gap_idx[count]][50:-150]], ax=mid_lower_axes[count], cmap='Blues', vmin=0, vmax=1, cbar=False)
            
            # Right panel: 
            sns.heatmap(Align_PC_Sign(onset_post_projection[:PCs,50:-150]), ax = right_upper_axes[count], cmap = 'RdBu', cbar = False)
            sns.heatmap([self.group.gaps_label[gap_idx[count]][50:-150]], ax=right_lower_axes[count], cmap='Blues', vmin=0, vmax=1, cbar=False)
        ...
        sim = np.zeros(10)
        gap_idx = np.arange(10)
        dur = 100
        for count in range(10):
            gap_dur = round(self.group.gaps[gap_idx[count]]*1000+350)

            onset = self.group.pop_response_stand[:, gap_idx[count],100:100+dur] # first 100 ms of pre-gap noise
            onset_post = self.group.pop_response_stand[:,gap_idx[count],gap_dur:gap_dur+dur] # first 100 ms of post-gap noise
            offset = self.group.pop_response_stand[:,i,gap_dur + 100: gap_dur + 100 + dur] # first 100 ms of post-gap silence

            period1, period2 = onset, onset_post
            period1_pca = neuron.PCA(period1, multiple_gaps = False)
            period2_pca = neuron.PCA(period2, multiple_gaps = False)
            angles = calculate_principal_angles(period1_pca.loading[:5].T, period2_pca.loading[:5].T)
            sim[count] = 1 - np.mean(angles) / (np.pi/2)

        axs.plot(np.arange(10), sim)
    ...
    def Plot_Noise_Return_Silence(self, dim = 3):
        ...
        onset_pca= neuron.PCA(self.group.pop_response_stand[:,0, 100:200], multiple_gaps=False)
        offset_pca= neuron.PCA(self.group.pop_response_stand[:,0, 450:550], multiple_gaps=False)
        noise_pca= neuron.PCA(self.group.pop_response_stand[:,0, 350:450], multiple_gaps=False)
        background_pca= neuron.PCA(self.group.pop_response_stand[:,0, 0:99], multiple_gaps=False)
...
class DynamicalSystem_Simple:
    def __init__(self, group, gap_idx):
        ...
        self.OnS_D, self.OffS_D = None, None
    ...
    def Optimize_Params(self):
        ...
        gap_labels = np.array(self.group.gaps_label[self.gap_idx]) 
        starts = [0]
        start = 0
        for i in range(1, len(self.times)):
            if gap_labels[i] != gap_labels[i-1]: start = i 
            starts.append(start)
            ...
            for iter in range(num_iterations):
                ...
                for t in range(100, len(self.times)):
                    if OnS[t] == 1:  # Sound is on, use tau_on
                        ...
                        OffS_D[t] = exponential_decay(starts[t], torch.tensor(t, dtype=torch.float32), tau_off*50)
                ...
                for t in range(1, len(self.times)):
                    ...
                    N[:, [t]] = N[:, [t-1]] + (W @ N[:, [t-1]] + OnRe * OnS_D[t-1] + OffRe * OffS_D[t-1]) * Nt + noise
                    pbar.update(1)
    ...
    def Run(self): 
        ...
        for t in range(100, len(self.times)):
            if self.OnS[t] == 1:  # Sound is on, use tau_on
                ...
                self.OffS_D[t] = exponential_decay_np(starts[t], t, self.tau_off)
               
        for t in range(1,len(self.times)):
            noise = np.random.normal(0, np.sqrt(0.001), (3, 1))
            self.N[:,[t]] = self.N[:,[t-1]] +(self.W@self.N[:,[t-1]] + self.OnRe*self.OnS_D[t-1] + self.OffRe*self.OffS_D[t-1] )*self.Nt + noise
